...
SOURCE_DESCRIPTORS: dict[str, SourceDescriptor] = {
    ...
    "uworld_notes": SourceDescriptor(
        ...
        asset_policy="none",
        cache_policy="source_hash",
        notes="High-yield UWorld notes profile (v4.59). Emits normalized text chunks before invoking the existing UWorld generator; the user confirmed UWorld notes are text-only so this source skips all image/table machinery. Default density retuned to ~1 question per 150 chars with a floor of 8 questions after the user's first live import — a 3-page UWorld note lands at ~9-10 questions for rigorous testing of high-yield Step 2 material.",
        metadata={
            ...
            "densityPolicy": "auto-scale --questions-per-file to max(8, chars // 150) clamped at 80; override via --questions-per-file",
