...
| NBME | v4.61 dual-PDF orchestrator (Q-PDF + A-PDF separate); auto-detects file roles, screenshot mode, A-PDF format; replaces the v4.60 per-stage chunker | Live Gemini end-to-end field-validated on `Medicalstudyzone.com Internal Medicine 3` Q+A pair (50/50 questions imported, q9 gap-recovered, q20 + q26 12-option matching sets, q28 + q29 10-option matching sets, q30 tabular with column-prefixed choices, q39 + q42 real clinical images auto-attached) | Tier-1 deterministic two-column rescue handles matching sets free; Tier-2 Gemini multimodal extraction handles tabular + ambiguous formats; gap recovery probes single-page OCR-missed boundaries | Live BIC end-to-end through packaged `.app`; runtime ~13 min for 50-Q dual-PDF (always-polish call adds canonical retrievalTag, reviewPearl, educationalObjective to every question) | Packaged `.app` live BIC run validated on Internal Medicine 3 Q+A pair | Screenshot-only NBME PDFs (no text layer) not yet stressed beyond per-page auto-detect; multi-question-per-page layouts unsupported; very large matching sets beyond 14 options (A-N) untested | Medium |
...
| UWorld Notes | v4.59 BIC profile registered (text-only, no images) | Live Gemini generation field-validated on real `Test uWorld.docx` (3-page note) through the packaged `.app`; user confirmed "everything works perfectly" | Shared normalized text chunks emitted via the same UWorld heading-aware splitter the downstream generator uses | Live BIC path wired; auto-import target directory registered in registry; dropdown entry added to renderer | Packaged `.app` live run validated on `Test uWorld.docx` (9 questions on 1,365 chars after density retune) | Broad input-format variance (.docx with images/complex tables, .rtf, very large multi-thousand-line notes that hit the 80-question MAX clamp) not stressed | Medium |
...
Newly validated at v4.60 (two-tier question-boundary regex + automatic stem-image attachment):

- `tools/nbme-pdf-json-generator/extract_pdfs.py` now splits the question-boundary regex into two tiers. Tier 1 (`_Q_BOUNDARY_STRONG_RE`) matches `Item N of M` anywhere on the line — the NBME Self-Assessment header that appears once per question page, even when wrapped inside a longer banner like `Exam Section : Item 1 of 50 National Board of Medical Examiners`. Tier 2 (`_Q_BOUNDARY_FALLBACK_RE`) preserves the legacy line-start patterns and adds handling for OCR-noise bullet characters (`~`, `*`, `•`, `·`, `●`, etc.) before stem numbers. The user reported a live BIC import failure on `[Medicalstudyzone.com] Internal Medicine 3 - Answers.pdf` with `"No question boundaries found — cannot chunk"`; v4.60 fixes that by promoting the `Item N of M` anchor to a mid-line strong match.
- `tools/nbme-pdf-json-generator/extract_pdfs.py` `chunk_raw_text` now tries the strong tier first and falls back to the weaker patterns only when the strong tier produces no matches. Avoids the double-match degenerate case where one question would be picked up by BOTH the header AND the stem bullet (would have produced one empty header-only chunk + one real chunk).
- `tools/nbme-pdf-json-generator/normalized_to_app_json.py` `convert_normalized_file` now accepts both `items` and `questions` as the top-level key. The dry-run normalization path historically wrote `"questions"` while the live Gemini path wrote `"items"`; the converter only read `items`, so dry-run end-to-end never produced an app-ready JSON. Pre-existing bug, surfaced by v4.60 testing.
- `tools/nbme-pdf-json-generator/nbme_extract_figures.py` gains `auto_attach_figures_to_app_ready` — writes figure crops directly into `q.images[]` with `figureKey: null` + inline base64 `dataUrl`, mirroring the v4.58 Mehlman / v4.56 images-tables app-ready contract. Mirrors the existing `build_suggested_figure_links` logic but actually mutates the app-ready JSON instead of just emitting suggestions HTML. NBME explanations have no images (user confirmed) so all attachments go to `placement: stem`. No Gemini multimodal calls — pure file-system attachment.
- Aspect-ratio guard built into the auto-attach helper (`AUTO_ATTACH_MIN_ASPECT=0.30`, `AUTO_ATTACH_MAX_ASPECT=3.00`). Real clinical images (EKGs, X-rays, derm photos, gross pathology, histology slides) cluster between 0.5:1 and 2.5:1. Text-block false positives from the rendered-page CV detector consistently sit at 3.5:1 to 8.6:1. The 3.0 ceiling cleanly separates real images from text blocks on the user's `Internal Medicine 3 - Questions.pdf` — 12 of 14 candidates filtered out (all wide text strips), 2 admitted (the 0.9 and 2.2 aspect-ratio candidates).
- `tools/nbme-pdf-json-generator/nbme_batch_wrapper.py` `run_app_ready` now invokes `auto_attach_figures_to_app_ready` after the existing `build_suggested_figure_links` call. Default threshold is `min_confidence="medium"`. Low-confidence candidates still surface in the existing review HTML, so the user can manually review edge cases — but does NOT have to manually crop every image just to get the obvious ones attached.
- Field-verified offline through the packaged `.app` on the user's `Internal Medicine 3 - Questions.pdf` (10 pages, 10,960 chars): OCR + chunking produces 9 chunks (was 0 before v4.60). Live Gemini normalization is wired but not field-stressed in v4.60 — it uses the same code path Mehlman / OME / Anki use, which has been live-validated since v4.51.

Newly validated at v4.61 (dual-PDF orchestrator — replaces the per-stage v4.60 chunker for fire-and-forget NBME imports):

- New `tools/nbme-pdf-json-generator/nbme_dual_pdf_runner.py` is now the single BIC entrypoint for NBME. Replaces the prior four-stage `nbme_batch_wrapper.py` pipeline (OCR / chunking / normalization / app-ready) with one orchestrator that handles three input modes auto-detected from the upload: dual PDF (Q-PDF + A-PDF separate), single Q-only PDF (Gemini generates explanations), and single combined PDF (legacy v4.60 path retained).
- File-role detection via filename keywords (`Questions`/`Answers`, `_Q_`/`_A_`) plus content sniff (`Correct Answer:` phrase presence, NBME chrome). Auto-resolves which uploaded PDF is the Q-PDF vs A-PDF. Two-file upload defaults to dual mode; single-file upload falls through to combined or Q-only based on inline-answer detection.
- Screenshot-mode auto-detection per PDF: if `pdfplumber` returns <100 useful chars on more than 50% of the first 10 pages, the PDF is treated as a screenshot and downstream processing relies on tesseract OCR.
- Two-tier suspicious-chunk handling. Tier 1 (`deterministic_multi_column_parse`) is a free regex pass that catches multi-column matching sets — finds every `[A-N])` marker, splits text between consecutive markers, sorts choices alphabetically. Handles q20-style 11-option sets, q26-style 12-option sets, q28/q29-style 10-option sets — all without Gemini. Tier 2 is Gemini multimodal page extraction for the formats deterministic can't handle (tabular choices like q30 with column headers).
- Gap-recovery for OCR-missed boundaries. When a question number is missing between two known neighbours AND the page gap is exactly 2 (one page between them), the orchestrator renders that page and calls Gemini multimodal with an unbiased prompt — "extract whatever question is on this page" rather than "extract question N". Verified on Internal Medicine 3 where q9's `Item 9 of 50` header had been mangled by OCR; q9 was recovered cleanly with the correct stem + choices.
- Letter range A-J → A-N (12-14 option matching sets). NBME ships matching sets up to 12 (q20, q26) and occasionally larger. v4.61 raises every letter-pattern regex (`_CHOICE_LINE_RE`, `_CORRECT_ANSWER_RE`, `deterministic_multi_column_parse`, suspicion-detector leakage check, multimodal prompts) to `[A-N]`.
- Smart-trigger Gemini multimodal figure detection — only fires on pages where the stem matches the strict image-language regex (specific phrases like "the photograph", "the EKG", "peripheral blood smear") OR PyMuPDF reports an embedded raster of ≥400×400 px AND ≥150K px² area. Cuts figure-detection calls from 49 (v4.60-day-1 loose trigger) to ~10 per typical 50-Q PDF.
- Fractional bbox + 1200-px page render — fixes the v4.60 image-cropping bug where Gemini returned bbox coordinates in its own downscaled space (~1024 px wide) but my crop code applied them to a full-resolution 5689×5467 px render, producing 275-px-wide crops of NBME header chrome rather than the actual clinical image. v4.61 renders at exactly 1200 px wide and asks Gemini for fractional bbox (0.0–1.0); the crop then multiplies fractions × actual image dimensions. Field-verified: q39's "peripheral blood smear" and q42's "dermatology image of skin lesion" now produce 337×473 px and 492×422 px crops of the actual clinical content, not header noise.
- Bbox safety guards: reject any bbox with y1 < 0.08 (NBME header) OR y2 > 0.94 (footer chrome) OR area < 3% (too small) OR area > 75% (whole-page-as-image trap). Aspect-ratio guard (0.30–3.00) inherited from v4.60.
- Always-polish canonical call. After per-question assembly (regardless of whether explanation came from A-PDF parse, combined-mode inline parse, or Gemini completion), run one more Gemini text call to produce canonical `explanationSections`, `reviewPearl`, `retrievalTag`, `educationalObjective`. Replaces the prior happy-path string-splitting that left these fields as placeholders ("Refer to the explanation.", "Apply the tested clinical reasoning."). Polish uses `max_tokens=4096` to accommodate 10+ choice matching sets' incorrect-explanation body. On polish failure, a salvage Gemini text call asks for ONLY the three meta fields (small response, less truncation risk).
- Aggressive A-PDF explanation chrome cleanup (`_clean_explanation_chrome`). Strips `Exam Section :`, `National Board of Medical Examiners`, `Mark Medicine Self-Assessment`, the timer line, all NBME footer UI chrome (`Next / Score Report / Lab Values / Calculator / Help / Pause / Previous / Review`), URL footers, OCR-noise fragments (`r ,,,`, `~ ~`, `" ' ,`, `r--`), and the ■ checkbox glyph. Applied in all three A-PDF parse paths (NBME interface, plain text, combined-mode inline).
- Renderer fix in `index.html`: `#q-stem`, `.shared-group-stem`, `.question-individual-stem`, and `.ngj-exp-section p` now have `white-space: pre-wrap` so the multi-line lab-value blocks NBME pages contain (`Prothrombin time 18 sec` / `Serum / Protein / Total 8.0 g/dL` / etc.) render with their original line breaks rather than collapsing to flat prose.
- Stub-with-warning last-resort fallback: when every extraction tier fails, the question is still emitted with whatever stem and choices we have plus `extractionWarnings`, instead of silently being dropped from the import. User can manually review.
- BIC `pipeline_registry.json` simplified to a single-step entry calling the orchestrator; the prior four-stage cascade is removed. Both `dryRunSteps` and `liveSteps` now point at `nbme_dual_pdf_runner.py --input-file {inputFile}`. BIC iterates inputs and the orchestrator dedupes via a completion-marker flag so duplicate iterations no-op.

Field-validated live BIC end-to-end through the packaged `.app` on the user's `Medicalstudyzone.com Internal Medicine 3 - Questions.pdf + - Answers.pdf` pair (50 questions): 50/50 imported, q9 gap-recovered, q20 (12 options A-E + G-L, spurious OCR-empty F filtered), q26 (12 options A-L), q28 + q29 (10 options A-J with shared instruction line preserved), q30 (tabular with column-prefixed choice text: `A) Specific Gravity 1.003 | Glucose - | Protein 1+ | WBC 30 | RBC 5 | Casts muddy brown | Findings tubular epithelial cells`), q39 + q42 (real clinical images attached). Runtime ~13 min for the full 50-Q dual-PDF (~58 Gemini calls — one polish per question dominates). Cost ~$0.30 per import.

Not yet validated at v4.61:

- Pure screenshot NBME PDFs (no text layer at all on most pages). Screenshot auto-detection runs but downstream extraction quality on heavily-OCR'd screenshot pages is untested beyond the synthetic checks.
- Multi-question-per-page NBME exports — orchestrator assumes one question per page when inferring page→question mapping for gap recovery and figure attachment.
- Matching sets beyond 14 options (A-N). Range goes to N, but if NBME ships an A-P or A-T format somewhere the regex would need to widen.
- A-PDF order changes (q13 → q4 → q28 layout that the user mentioned exists in some NBME PDFs). The orchestrator joins by question number so this *should* work, but it hasn't been live-stressed against an out-of-order A-PDF.
- Behavior under Gemini quota exhaustion mid-NBME-import. The orchestrator has a hard ceiling of 500 Gemini calls per run as a runaway-cost guard, but quota-aware retry stop (v4.54 style) is not yet wired into this orchestrator's call sites.

Not claimed (carried forward from prior NBME milestones):
...
Three v4.59 follow-up commits landed during field validation, all on top of the original `v4.59-uworld-live-stable` tag (no new tag — these are tuning + bug fixes, not architectural changes):

1. **Renderer dropdown fix ([e14e811](https://github.com/shamsulalamx/shamsulalamx/commit/e14e811))**: the BIC source dropdown in `index.html` is a hardcoded `<option>` list, not a registry-driven enumeration. The original v4.59 source commit wired the backend completely but the dropdown didn't surface "UWorld Notes" until two index.html edits landed (added `<option value="uworld_notes">UWorld Notes</option>` to the dropdown at line 2349 and a matching entry in `BATCH_IMPORT_SOURCE_LABELS` at line 8621).
2. **Missing-dep fast-fail + `.docx` byte-count bug ([cc8af39](https://github.com/shamsulalamx/shamsulalamx/commit/cc8af39))**: first live UWorld import on `Test uWorld.docx` failed with the cryptic "did not produce expected app-ready JSON" error because (a) `python-docx` was not installed in the system Python the BIC subprocesses use, so `extract_text` silently returned empty and the downstream generator skipped the file, and (b) the auto-density math used `stat().st_size` for `.docx` which overcounts ~40× because `.docx` is a zipped XML container (57 KB file ≈ 1.4 KB of actual text). Fixed by extracting text once up front via the real UWorld extractor for an accurate char count AND pre-flighting the dep check so missing `python-docx` / `striprtf` surface a clear `pip install --user` message instead of a downstream filesystem error. New `requirements.txt` at repo root documents the full system-Python dep set (`pdfplumber`, `PyMuPDF`, `python-docx`, `striprtf`, `python-pptx`, `Pillow`) since electron-builder does not bundle Python.
3. **Density retune for higher rigor ([5495bff](https://github.com/shamsulalamx/shamsulalamx/commit/5495bff))**: first live import on the user's 3-page UWorld note produced 5 questions ("THESE ARE HIGH YIELD MATERIAL, and I would want more rigorous testing"). The day-1 floor (MIN=5, DEFAULT=500 chars/q) dominated on small files. Retuned to MIN=8 + DEFAULT=150 chars/q → 1.4 KB note now lands at 9 questions, 4 KB at 27, 10 KB at 66, 12+ KB at the MAX clamp of 80 (cost cap stays ~$0.16/file).

Field-validated end-to-end through the packaged `.app` on `/Users/shamsulalam/Desktop/Test uWorld.docx` (a real 3-page user UWorld note): `extractedChars=1365`, `questionsPerFile=9`, `chunkCount=1`, `candidateQuestionCount=9`, `outcome: completed`, no chunk failures. User confirmed "everything works perfectly."

Also offline-validated:
- 1.8 K `test_cardiology.txt` → 8 questions (MIN clamp under the new density).
- 4.1 K synthetic 5-section UWorld file → 27 questions (auto-scaled).

Not yet stressed:

- `.docx` files with complex formatting (embedded images, complex tables, multiple sections with formatting overrides).
- `.rtf` files (the `striprtf` path is wired but not field-exercised).
- Very large UWorld notes (>12 K chars where the MAX clamp of 80 questions engages and effective density drops back to ~1 q per 150 chars).
- Behavior under Gemini quota exhaustion mid-import (inherited from the v4.54 UWorld-family quota-aware retry stop; not stressed for v4.59 specifically).
