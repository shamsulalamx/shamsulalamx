...
| NBME | BIC orchestration stable; v4.60 two-tier question-boundary regex + auto-attach stem images | Existing NBME pipeline stable for known workflows; v4.60 unblocks NBME Self-Assessment PDFs with `Exam Section : Item N of 50 ...` headers (the boundary detection used to fail because `Item N` appeared mid-line, not line-start) | Adapter foundation exists | BIC orchestration tagged stable; v4.60 auto-attaches high/medium-confidence stem images into `q.images[]` so users do not have to manually crop every figure through the cropper UI | Packaged `.app` validated on `Medicalstudyzone.com Internal Medicine 3` Questions + Answers PDFs (boundary regex matches and aspect-ratio guard rejects text-block false positives offline) | Live Gemini run on a real NBME PDF with embedded clinical images not yet field-validated; aspect-ratio guard at 3.0 may admit borderline text blocks on edge cases (user can manually remove) | Medium |
...
- All NBME forms.
- Perfect OCR.
- Live Gemini run on a real NBME PDF with embedded clinical images — only the chunking + auto-attach offline path has been field-validated in v4.60. The live Gemini call signature is unchanged from prior NBME work, but the auto-attach + new boundary regex have not yet been driven by a live import.
- The aspect-ratio guard at 3.0 — borderline cases (e.g., wide EKG strips around 3:1, narrow text blocks just under 3:1) may misclassify in either direction. User can manually remove false-positive attachments via the app's existing image management UI; misses can be recovered through the still-present review HTML.
