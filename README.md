...
| Images & Tables | Screenshots `.png / .jpg / .jpeg / .webp / .bmp / .tif / .tiff` | Yes (live per-image Gemini classification + NBME-style question generation through BIC since v4.56) |
...
| `v4.56-images-tables-live-stable` | Enabled live Gemini per-image classification + NBME-style question generation for the Images & Tables BIC source, replacing the v4.15 attachment-first boilerplate. Tightened the classifier so tables and charts never appear in the question stem (prompt rule + post-classification override). Fixed two follow-on bugs surfaced by the first live BIC runs: (a) multi-file selections now produce a single combined `images_tables_combined_app_ready.json` that accumulates across BIC's per-input invocations using a stable filename and `*_per_image.json` debug artifacts that fall outside discovery; (b) the renderer no longer double-renders the explanation panel when both `correctBlurb` and `explanation` are populated. Field-validated on a 5-image packaged-app BIC run with mixed diagnostic / explanation-only / table inputs. |
| `v4.57-amboss-deterministic-hybrid-stable` | Full rewrite of the AMBOSS PDF BIC extractor as a deterministic-first, Gemini-assisted pipeline. Replaces the prior per-page Gemini extraction (~$0.50 / ~9 min per PDF) with tesseract OCR for page classification + stem fingerprinting + nav-pill grouping + adjacent-component merge, then ONE Gemini call per question for clean choices / correct answer / per-choice explanations / educational objective / retrieval tag / review pearl. Adds post-extraction stem dedupe (handles OCR variance that fragments one question into two) and a Gemini recovery fallback for groups where deterministic extraction returned None. Uses `responseMimeType: application/json` + `maxOutputTokens: 8192` to eliminate truncated-JSON failure modes. Field-validated on a 39-page / 8-question AMBOSS QBank PDF: all 8 questions extracted with clean choices, correct answers, and clinical images routed to explanations. Runtime ~2.5 min, cost ~$0.11 per import. |
